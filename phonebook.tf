...
provider "aws" {
  region = "us-east-1"            ## Buraya yazmamiza gerek kalmamis olacak.
  //  access_key = ""             ## Jenkins kullanacagi icin ona geerkli yetkileri verecegiz.
  //  secret_key = ""
...
resource "aws_iam_instance_profile" "ec2ecr-profile" {
  name = "oliverswarmprofile"                           ## Rol isimleri degistirilebilir.
...
resource "aws_instance" "docker-machine-leader-manager" {
  ...
  security_groups = ["oliver-docker-swarm-sec-gr"]        ## ismi degistirebiliriz ama diger yerlerde degismeli.
  iam_instance_profile = aws_iam_instance_profile.ec2ecr-profile.name
  user_data = data.template_file.leader-master.rendered
  tags = {
    Name = "oliver-Docker-Swarm-Leader-Manager"         ## yine isimi degistirilebilir.
...
resource "aws_security_group" "tf-docker-sec-gr" {
  name = "oliver-docker-swarm-sec-gr"                 ## Docker-Swaarm hangi portlarin acilmasi gerekiyor veriyor
  tags = {
    Name = "swarm-sec-gr"
  }
  dynamic "ingress" {
